...
PHASE_CONFIGS = {
    JPPPhase.PLANNING_INITIATION: {
        ...
        "topic": "Establish planning organization, review strategic guidance, and frame the operational problem",
        "lead_agents": [StaffRole.J5, StaffRole.J3, StaffRole.J2],
        "key_outputs": [
            "Higher HQ Direction & Constraints (bullets)",
            "Initial Planning Assumptions (min 5, with basis & impact-if-wrong)",
            "Problem Framing (1 paragraph)",
            "Tasks to Staff (J2/J3/J5/J6 minimum, each with WHO/WHAT/WHEN/SUCCESS)",
            "Initial Information Requirements",
        ],
        "min_turns": 15,
        "focus_areas": [
            "Strategic guidance interpretation and constraints from higher HQ",
            "Problem framing: 1 paragraph, no generic filler",
            "At least 5 planning assumptions with validation status",
            "Initial tasks assigned to J2, J3, J5, J6 with success conditions",
            "Information requirements that will become CCIRs in Step 2",
        ],
        "artifact_class": "Step1PlanningInitiation",
    },
    JPPPhase.MISSION_ANALYSIS: {
        "name": "Mission Analysis",
        "topic": "Analyze the mission, produce restated mission, CCIRs, COG analysis, and running estimates",
        "lead_agents": [StaffRole.J2, StaffRole.J3, StaffRole.J5],
        "key_outputs": [
            "Mission Statement (joint format: who/what/when/where/why)",
            "Commander's Intent Draft (purpose/method/end state)",
            "Operational Limitations: restraints & constraints",
            "Specified / Implied / Essential Tasks",
            "Centers of Gravity (friendly + adversary) with CC/CR/CV",
            "Draft CCIRs: PIR (>=3) / FFIR (>=3) / EEFI (>=3)",
            "ISR/Collection Outline mapped to PIRs",
            "Running Estimates by J2/J3/J4/J5/J6",
        ],
        "min_turns": 20,
        "focus_areas": [
            "METT-TC analysis grounded in specific data, not vague summaries",
            "COG analysis with critical factors (CC, CR, CV) for both sides",
            "3+ PIR, 3+ FFIR, 3+ EEFI — each with indicator and collection asset",
            "ISR outline: which asset collects against which PIR, by when",
            "Running estimates must be short but specific, not generic",
            "Risk entries must have probability + severity + narrative + mitigation",
        ],
        "artifact_class": "Step2MissionAnalysis",
    },
    JPPPhase.COA_DEVELOPMENT: {
        "name": "COA Development",
        "topic": "Develop minimum 3 distinct COAs with phasing, task org, cyber, sustainment, and C2",
        "lead_agents": [StaffRole.J5, StaffRole.J3, StaffRole.FIRES],
        "key_outputs": [
            ">=3 DISTINCT COAs, each with:",
            "  Operational approach (lines of operation/effort)",
            "  Phasing (Phase 0-III+) with key tasks per phase",
            "  Main effort + supporting efforts",
            "  Task organization / command relationships",
            "  Cyber integration by phase (DCO, OCO, EMS, ISR)",
            "  Sustainment concept (J4) and Comms/C2 concept (J6)",
        ],
        "min_turns": 25,
        "focus_areas": [
            "Each COA must be FEASIBLE, ACCEPTABLE, SUITABLE, DISTINGUISHABLE",
            "Phasing must specify key tasks, not just descriptions",
            "Cyber integration by phase: DCO, OCO, EMS, ISR linkages",
            "J4 sustainment and J6 C2 concept per COA, not generic",
            "Task org must specify supported/supporting relationships",
            "Reference Step 2 CCIRs and COG analysis by ID",
        ],
        "artifact_class": "Step3COADevelopment",
    },
    JPPPhase.COA_ANALYSIS: {
        "name": "COA Analysis & Wargaming",
        "topic": "Wargame each COA with Action-Reaction-Counteraction tables and decision points",
        "lead_agents": [StaffRole.J2, StaffRole.J3, StaffRole.FIRES],
        "key_outputs": [
            "Wargame method declared (belt/box/avenue-in-depth)",
            "ARC table per COA (>=4 entries each)",
            ">=4 Decision Points with trigger, decision, branch/sequel, owner",
            "Updated risk per COA (prob/sev + mitigation)",
            "Updated CCIRs if changed",
            "Collection gaps identified",
        ],
        "min_turns": 25,
        "focus_areas": [
            "Declare wargame method explicitly",
            "ARC entries must reference specific phases from Step 3",
            "Decision points must have concrete trigger conditions",
            "Each DP links to a CCIR (PIR/FFIR) from Step 2",
            "Identify strengths AND weaknesses per COA",
            "Recommended modifications based on wargame findings",
        ],
        "artifact_class": "Step4COAAnalysis",
    },
    JPPPhase.COA_COMPARISON: {
        "name": "COA Comparison",
        "topic": "Compare COAs using weighted criteria scoring matrix and recommend preferred COA",
        "lead_agents": [StaffRole.J5, StaffRole.J3, StaffRole.SJA],
        "key_outputs": [
            "Weighted comparison criteria (>=6 criteria with weights)",
            "COA scoring matrix (numeric scores per criterion per COA)",
            "Sensitivity notes (what changes the recommendation)",
            "Recommended COA with justification tied to scores + risk",
        ],
        "min_turns": 20,
        "focus_areas": [
            "At least 6 weighted criteria with numeric weights",
            "Numeric scores (1-10) for each COA against each criterion",
            "Weighted totals must be correctly calculated",
            "Sensitivity analysis: what assumption change flips the ranking",
            "Justification must reference specific scores and wargame findings",
        ],
        "artifact_class": "Step5COAComparison",
    },
    JPPPhase.COA_APPROVAL: {
        "name": "COA Approval",
        "topic": "Commander approves COA, finalizes intent and CCIRs, accepts risk, issues planning guidance",
        "lead_agents": [StaffRole.J5, StaffRole.J3, StaffRole.COMMANDER],
        "key_outputs": [
            "Commander's COA decision statement",
            "Refinements required to selected COA",
            "Finalized Commander's Intent (purpose/method/end state)",
            "Finalized CCIRs (PIR/FFIR/EEFI)",
            "Risk acceptance statement + delegated authorities",
            "Planning guidance for OPORD development",
        ],
        "min_turns": 15,
        "focus_areas": [
            "Commander makes a DECISION, not a recommendation",
            "Refinements must be specific and actionable",
            "Finalized intent refined based on wargaming results",
            "CCIRs updated based on wargaming decision points",
            "Risk acceptance must be specific (what risk, why accepted)",
            "Planning guidance with priorities, timelines, focus areas",
        ],
        "artifact_class": "Step6COAApproval",
    },
    JPPPhase.PLAN_DEVELOPMENT: {
        "name": "Plan/Order Development",
        "topic": "Develop draft OPORD with all 5 paragraphs, annexes, sync matrix, DSM, and assessment plan",
        "lead_agents": [StaffRole.J3, StaffRole.J5, StaffRole.J4],
        "key_outputs": [
            "Draft OPORD (5-paragraph order: Situation, Mission, Execution, Admin/Log, Cmd/Sig)",
            "  Each task: TASK + PURPOSE + NLT + COORD + SUCCESS CONDITION",
            "Annex stubs: B (Intel/ISR), C (Ops/Fires/Cyber), K (Comms/Cyber Defense)",
            "Synchronization Matrix (>=8 rows: D-7 through D+2)",
            "Decision Support Matrix (DP#, trigger, decision, branch, owner)",
            "Assessment Plan: MOE/MOP (metric, method, frequency, owner, threshold)",
        ],
        "min_turns": 25,
        "focus_areas": [
            "Tasks to subordinates: TASK + PURPOSE + NLT + COORD + SUCCESS",
            "Coordinating instructions: ROE, reporting, deconfliction, authorities",
            "Sync matrix rows: maneuver, fires, intel, logistics, cyber/EW, protection, IO",
            "DSM entries directly from Step 4 decision points",
            "Assessment plan: both MOE (effectiveness) and MOP (performance)",
            "Annex B must map ISR assets to PIRs from finalized CCIRs",
        ],
        "artifact_class": "Step7PlanDevelopment",
    },
}
